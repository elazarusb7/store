...
class SamhsaPepNotification extends FormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    try {
      ...
      $id = $connection->merge('samhsa_pep_notification_history')->fields([
          'uid' => $uid,
          'sid' => session_id(),
    ...
    Drupal::messenger()
      ->addMessage($this->t($this->config('samhsa_pep_notification.settings')
        ->get('success')));
    $msg = Drupal::service('samhsa_pep_utility.pep_utility_functions')
      ->showRequiredTfaMsg($uid);
    if (strlen($msg) > 0) {
  ...
  private function getRedirectionUrl() {
    ...
    if (isset($current_user_roles[1])) {
      ...
      $cart = Drupal::service('commerce_cart.cart_provider')->getCarts();
      if (count($cart)) {
