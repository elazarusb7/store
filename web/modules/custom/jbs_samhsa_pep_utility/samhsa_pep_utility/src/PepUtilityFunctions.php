...
/**
 * @file
 * Contains \Drupal\samhsa_pep_utility\PepUtilityFunctions.
 */
...
class PepUtilityFunctions implements PepUtilityFunctionsInterface {
  ...
  public function checkFilters($array){
        $counter = 0;
        $key_exclude = array('submit', 'reset', 'form_build_id', 'form_id', '');
        foreach($array as $key => $value){
            if(is_array($value)){
                $counter += $this->checkFilters($value);
            } else if (!in_array($key, $key_exclude)  &&
  ...
  function format_phone_string($phoneNumber) {
     ...
     * {@inheritdoc}
     */
    function _get_vocabulary_as_select_options($vid) {
        $terms = \Drupal::entityTypeManager()->getStorage('taxonomy_term')->loadTree($vid);
        $result = array('' => '- Select product location -');
     ...
     * {@inheritdoc}
     */
    function _get_vocabulary_as_select_options_by_variation_id($variation_id) {
        // Print the targeted entity type field.
        $variation = \Drupal\commerce_product\Entity\ProductVariation::load((int)$variation_id);
     ...
     * {@inheritdoc}
     */
    function recalculateWeight(\Drupal\commerce_order\Entity\Order $order)
    {
        $totalweight = 0.00;
        if ($order->hasItems()) {
     ...
     * {@inheritdoc}
     */
    function recalculateShipmentWeight(\Drupal\commerce_shipping\Entity\Shipment $shipment){
        $totalweight = 0.00;
     ...
     * {@inheritdoc}
     */
    function showRequiredTfaMsg($uid){
        $msg = '';
